...
const createBanks = () => {
    let banksList = banks.map(bank => new Bank(bank.id, bank.name));
    return banksList;
};

// const createClients = () => {
//     let clientsList = clients.map(client => new Client(client.id, client.taxNumber, client.name));
//     return clientsList;
// }
...
const createClients = () => clients.map(client => {
    ...
    // console.log("🚀 ~ file: classes.js ~ line 251 ~ newClient", newClient)
    return newClient;
});
...
class ListSortClients {
    ...
    listClientsIdsSortByTaxNumber = () => clients.sort((clientA, clientB) => clientB.taxNumber - clientA.taxNumber).map(client => client.id);
